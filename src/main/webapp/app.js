...
function loadNavbar(){
	...
	xhr.onreadystatechange = function() {
		if(xhr.readyState == 4 && xhr.status ==200){
			...
			document.getElementById('revaturehome').addEventListener('click', loadDashboardView, false);
		}
	}
...
function retrieveBankUserAccountInfo(){
	...
	xhr.onreadystatechange = function() {
		if(xhr.readyState == 4 && xhr.status ==200){
			...
			document.getElementById('ersid').innerHTML = userDTO.username;
			document.getElementById('fname').innerHTML = userDTO.baId;
			document.getElementById('laname').innerHTML = userDTO.baBalance;
			document.getElementById('uname').innerHTML = userDTO.username;
			document.getElementById('pword').innerHTML = userDTO.baId;
			document.getElementById('ptype').innerHTML = userDTO.baBalance;
			ocument.getElementById('email').innerHTML = userDTO.baBalance;
		}
	}
